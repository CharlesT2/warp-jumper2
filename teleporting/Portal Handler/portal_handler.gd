...
var portal_jump_ready = true 
var portal_beam_ready = true
var beam_holder = []
...
func portaljump():
	if Input.is_action_just_pressed("Portal Jump") and portal_jump_ready:
		portal_jump_ready = false
		...
		portal_jump_ready = true
	pass


func portalbeam():
	if Input.is_action_just_pressed("Portal Beam") and portal_beam_ready and len(beam_holder) < 3:
		#portal_beam_ready = false
		var new_portal_beam = portal_beam.instantiate()
		new_portal_beam.global_position = $Marker2D.global_position
		beam_holder.append(new_portal_beam)
		new_portal_beam.handler = self
		get_tree().get_root().add_child(new_portal_beam)
		#if len(beam_holder > 3)
		#await get_tree().create_timer(5).timeout
		#portal_beam_ready = true
